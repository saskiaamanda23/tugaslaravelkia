...
<div class="card-body">
    <table id="castsTable" class="table table-bordered table-hover">
      <thead>
        <tr>
          <th>#</th>
          <th>Nama</th>
      ...
      <tbody>
        @foreach($casts as $cast)
          <tr>
            ...
            <td>{{ $cast->bio ?? '-' }}</td>
            <td>
              <form action="{{ route('casts.destroy', $cast->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')">
                @csrf
                @method('DELETE')
